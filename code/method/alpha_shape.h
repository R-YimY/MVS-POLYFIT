...
typedef CGAL::Triangulation_data_structure_2<Avb, Fb2>					Tds2;
typedef CGAL::Delaunay_triangulation_2<Kernel, Tds>						Dt;
typedef CGAL::Delaunay_triangulation_2<Kernel, Tds2>					Triangulation_2_;
typedef CGAL::Triangulation_hierarchy_2<Dt>								Ht;
typedef CGAL::Alpha_shape_2<Triangulation_2_>							Alpha_Shape_cgal;
...
inline std::vector<Face_handle> AlphaShape::extract_largest_solid_component() {
	...
	for (; it != ids.end(); ++it) {
		...
		int id = it->second;
		area[id] += std::fabs(CGAL::to_double(plg.area()));
	}
