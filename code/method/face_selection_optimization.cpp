	...
	std::vector<Plane3d*>& v, LinearProgramSolver::SolverName solver_name)
{
	...
	double Img_Iheight = Method::Image_max-Method::Image_min;//影像的差异值
    ...
    double coeff_data_fitting = Method::lambda_data_fitting;//拟合项
	double coeff_Image = total_points * Method::lambda_model_image / Img_Iheight;
	coeff_Image /= num_faces;
	...
	FOR_EACH_FACET(Map, model_, it)
	{
		...
		obj.add_coefficient(var_idx, coeff_Image * avg_z);
