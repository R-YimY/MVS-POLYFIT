...
typedef Delaunay_triangulation::Face_handle Delaunay_Face_handle;
...
//外部接口
//////////////////////////////////////////////////////////////////////////////////////
void Reconstruction::Initialize(PointSet* pset_, Map* foot_print_, std::vector<VertexGroup::Ptr> Group_) {
	...
	if (!foot_print_)
	{
		...
		foot_print = foot_print_;
	}


	if (!Group_.size())
	{
		Logger::warn("-") << "Plane Group data does not exist" << std::endl;
		return;
	}
	else {
		PlaneGroup = Group_;
	...
	std::vector<VertexGroup::Ptr> &groups_ = pset->groups();
...
void Reconstruction::Extract_Building_planes() {
...
bool Reconstruction::Reconstruct( Map *result, LinearProgramSolver::SolverName solver_name,
	bool update_display) {
	...
	FOR_EACH_FACET(Map, foot_print, it)
	{
		...
		PointSet::Ptr roof_pset = create_roof_point_set(roofs, g);
		PointSet::Ptr image_pset = create_projected_point_set(roof_pset);
...
void Reconstruction::Get_PlaneGroup( VertexGroup *building) {


	std::vector<unsigned int> p_index = *building;
	const std::vector<VertexGroup::Ptr> &planes_ = PlaneGroup;

	std::cout << "平面数量：" << planes_.size() << std::endl;

	for (std::size_t j = 0; j < planes_.size(); ++j)
	{
		const Color &c = random_color();
		planes_[j]->set_color(c);
	}

	building->set_children(planes_);

}


bool Reconstruction::is_simple_polygon(const Map::Facet *f)
{
...
PointSet * Reconstruction::create_roof_point_set(const std::vector<VertexGroup::Ptr> &segments, VertexGroup *building) {
...
PointSet *Reconstruction::create_projected_point_set( const PointSet *roof) {
...
std::vector<vec3>Reconstruction::compute_line_segment(PointSet *seg_pset, PointSet *roof_pset, Map::Facet *footprint) {
	...
	if (roof_pset->points().size() > 70)
	{
		...
		FT tolerance = 2.0;
		Image_edge_simplify otr2(image_x_min, image_y_min, image_delta_res);
...
std::vector<std::vector<int>> Reconstruction::compute_height_field(PointSet *pset, Map::Facet *footprint) 
{
	...
	double point_density = Method::image_resolution;
	...
	for (int j = 0; j < image_width; ++j)
	{
		for (int k = 0; k < image_width; ++k)
		{
			...
			{
				Delaunay_Face_handle fh = dt.locate(query_p);
	...
	LinearProgramSolver::SolverName solver_name)
{
	...
	PolyFitInfo polyfits;
	Map *hypothesis = hypo.generate(&polyfits, footprint, line_segments);
	if (!hypothesis)
		return nil;
	std::vector<Plane3d *> v = hypo.get_vertical_planes();
	// generate quality measures
	polyfits.generate(roof_pset, hypothesis, v, false);

	FaceSelection selector(roof_pset, hypothesis);

	selector.optimize(&polyfits, footprint, v, solver_name);

	extrude_boundary_to_ground(hypothesis, Geom::facet_plane(footprint), &polyfits);
...
Map *Reconstruction::generate_polygon(PointSet *pSet, double footprint_height, double density)
{
	...
	Method::image_resolution = density;
